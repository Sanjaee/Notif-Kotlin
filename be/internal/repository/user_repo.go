...
type UserRepository interface {
	...
	// ListForChat returns active users (exclude excludeUserID), for chat partner picker
	ListForChat(excludeUserID string, limit, offset int) ([]*model.User, error)
...
func (r *userRepository) ListForChat(excludeUserID string, limit, offset int) ([]*model.User, error) {
	if limit <= 0 {
		limit = 50
	}
	var users []*model.User
	q := r.db.Where("is_active = ?", true).Where("id != ?", excludeUserID)
	if err := q.Order("full_name").Limit(limit).Offset(offset).Find(&users).Error; err != nil {
		return nil, err
	}
	return users, nil
}
