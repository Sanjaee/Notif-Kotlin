...
func NewAuthService(userRepo repository.UserRepository, jwtSecret string, emailQueue EmailJobSubmitter) AuthService {
	return &authService{
		userRepo:   userRepo,
		jwtSecret:  jwtSecret,
...
func (s *authService) Register(req RegisterRequest) (*RegisterResponse, error) {
	...
	// Queue OTP email (non-blocking worker pool)
	...
	if !s.emailQueue.Submit(emailMsg) {
		log.Printf("Warning: OTP email queue rejected for %s", req.Email)
	}
...
func (s *authService) ResendOTP(email string) error {
	...
	if !s.emailQueue.Submit(emailMsg) {
		log.Printf("Warning: OTP email queue rejected for %s", email)
	}
	return nil
}
...
func (s *authService) RequestResetPassword(email string) error {
	...
	// Queue reset password email (non-blocking)
	emailMsg := util.EmailMessage{
		To:      email,
	...
	if !s.emailQueue.Submit(emailMsg) {
		log.Printf("Warning: reset password email queue rejected for %s", email)
	}
	return nil
}
